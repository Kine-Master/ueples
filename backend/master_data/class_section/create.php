...
try {
    ...
    $pdo->prepare("INSERT INTO class_section (grade_level_id, school_year_id, section_name, adviser_id) VALUES (?,?,?,?)")
        ->execute([$grade_level_id, $school_year_id, $section_name, $adviser_id]);
    $id = $pdo->lastInsertId();
    logAudit($_SESSION['user_id'], 'CREATE_CLASS_SECTION', "Created section '$section_name' (Grade #$grade_level_id, SY #$school_year_id)" . ($adviser_id ? " adviser=#$adviser_id" : ""));
